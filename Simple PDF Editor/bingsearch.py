...
def main():
    ...
    for each_login in range(len(usernames)):
        if usernames[each_login]:
            ...
            login(usernames[each_login], passwords[each_login], browser)
            for i in range(30):
                if i == 0:
                    time.sleep(1)
                    print(usernames[each_login] + " has " + get_points(browser) + " points.")
                do_search(random.choice(words) + " " + random.choice(words), browser)
